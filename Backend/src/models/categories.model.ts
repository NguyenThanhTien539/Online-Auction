...
export const getAllCategoriesLv2 = async (catId: number, catSlug : string) => {
    ...
    // Check catSlug match
    const checkData = await db.raw("select name from categories_1 where cat1_id = ?", [catId]);
    ...
    const result = await db.raw("select * from categories_2 where cat1_id = ?", [catId]);
    const data = result.rows;
    if (!data) {
        return null;
    }
...
export const getAllCategories = async () => {
    const result = await 
    db.raw("select cat1.cat1_id, cat1.name as cat1_name, cat2.cat2_id, cat2.name as cat2_name, cat2.image as cat2_image\
