...
export async function getUserRatingCount (req: Request, res: Response) {
    try {
        ...
        const ratingData = await userModel.getUserRatingCount({user_id, username});
        return res.status(200).json({
            status: "success",
            data: ratingData
        });
...
export async function getUserRatingHistory (req: Request, res: Response) {
    try{
        ...
        const ratingHistory = await userModel.getUserRatingHistory({user_id, username, offset, limit});
        return res.status(200).json({
            status: "success",
            data: ratingHistory
        });
