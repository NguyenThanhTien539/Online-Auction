...
function CatagoriesDropdownMenu(){
  ...
  return (
      ...
      catData.length === 0 ? null : <NavigationMenu className="min-w-[240px]">
              ...
              return(
                <div key={i}>
                  <NavigationMenuItem className="w-full flex ">
                    <NavigationMenuTrigger className="bg-transparent w-full font-semibold min-w-[240px] py-3 px-4 h-fit cursor-pointer hover:bg-gray-800/60 transition-all duration-100 rounded-lg text-white hover:text-gray-100 border border-transparent hover:border-gray-600/50 flex items-center justify-between"
...
function CatagoriesMiniItem({image, name, handleClick} : {image?: string, name?: string, handleClick : any}){
  ...
  return(
      <div className="group relative flex flex-col items-center">
        <div className = "h-[130px] aspect-square flex flex-col shrink-0 items-center justify-center rounded-2xl border-2 border-gray-600/60 \
          ...
          onClick = {handleClick}>
        {/* Image */}
        <img className = "object-cover h-[85%] aspect-square rounded-xl border border-gray-500/60 group-hover:border-gray-400/80 transition-all duration-500 shadow-lg" src = {image || Ronaldo} alt={name}></img>
      ...
      <div className = "text-center font-medium text-white/80 group-hover:text-white transition-all duration-300 text-sm tracking-wide">{name}</div>
      </div>
